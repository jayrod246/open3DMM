...
BOOL __export __loadds CALLBACK FBillbrdDlgProc(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK BBmpProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LPARAM lParam);
...
static HBITMAP FAR PASCAL LoadBitmapItem(HANDLE hInst, LPSTR lpName, BOOL *pfDefault);
...
static HFONT FAR PASCAL HfontSetBBFont(HWND hdlg)
{
    ...
    if ((hDlgFont = (HFONT)SendMessage(hdlg, WM_GETFONT, 0, 0L)) != hfontNull)
    {
        if (GetObject(hDlgFont, sizeof(LOGFONT), (LPSTR)&lFont))
        {
            ...
            {
                for (idc = IDC_BBTEXT1; GetDlgItem(hdlg, idc) != (HWND)NULL; ++idc)
                {
                    SendDlgItemMessage(hdlg, idc, WM_SETFONT, (WPARAM)hDlgFont, 0L);
                }
            }
        }
    }
...
**
****************************************************************************/
static LONG FAR PASCAL DrawDlgBmpItem(HWND hwnd, HDC hdc, LPSTR szBmp, DWORD rop, int fShadow, int fCalcSizeOnly,
                                      int DX, int DY)
{
    HDC hdcMem;
    HBITMAP hbmpSav;
    ...
    StretchBlt(hdc, rect.right - bmp.bmWidth - xw, rect.bottom - bmH - yw, bmp.bmWidth, bmH, hdcMem, 0, 0, bmp.bmWidth,
               bmp.bmHeight, rop);
...
-------------------------------------------------------------------------*/
long FAR PASCAL WndProcBillboardHeadline(HWND hwnd, WORD message, WORD wParam, LONG lParam)
{
    HDC hdc;
    PAINTSTRUCT ps;
    RECT rect;
        ...
        if (!fInitFont)
        {
                ...
                (cchWin = GetWindowsDirPath((char far *)szTtf, cchSzMax)) != 0)
            {
                ...
                if (DoesFileExist(szTtf, femExists))
                {
                    ...
                    RemoveFile(szFor, cmoForce);
                    fFontReady = CreateScalableFontResource(0, (char far *)szFor, (char far *)szTtf, NULL);
                }
            }
        }
        break;
...
static BOOL FAR PASCAL FRegisterWinClasses(VOID)
{
    ...
    wndclass.hbrBackground = hbrBillboard != NULL ? hbrBillboard : (HBRUSH)COLOR_WINDOW;
    ...
    wndclass.hIcon = NULL;
    wndclass.lpszMenuName = NULL;
    wndclass.hbrBackground = wndclass.hInstance = hinst;
    wndclass.style = CS_HREDRAW | CS_VREDRAW;
    wndclass.cbClsExtra = 0;
    wndclass.cbWndExtra = 0;
#endif
