...
*"EXPSETUP.DLL","FileCopy","213,NewName.Ext"					%224 *		ID#		"Error Title"	"Error Text"
*CustomAction	"EXPSETUP.DLL","FileMove","213,NewName.Ext"					%224
**
**	Author: ******
...
#define MAX_CAMF 41
static const char camfText[][24] = {"camfInitializeObject",
...
#define MAX_OIS 6
const char oisText[][27] = {"oisUnknown",       "oisNotInTree",   "oisNotYetInstalled",        "oisInstalledByUs",
                            "oisToBeInstalled", "oisToBeRemoved", "oisInstalledBySomebodyElse"};
        ...
        */
        ...
        if (DoesFileExist(((PCAMFDDoNonVisualMods)pcamfd)->lpvPriv, femExists))
        {
            if (DoesDirExist(pod->szDstDir))
            {
                ...
                {
                    char Text[300];
                    wsprintf(Text, "NewName='%s'\nDestDir='%s'\nCopyDest='%s'", szData, pod->szDstDir, szDestPath);
                    DoMsgBox(Text, "FileMove - DoNonVisualMods", MB_OK);
                }
#endif
