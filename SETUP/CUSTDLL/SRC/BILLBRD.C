...
#include "billbrd.h"
    ...
    BOOL __export __loadds CALLBACK
    FOffBlbrdDlgProc(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK OffBBmpProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LPARAM lParam);
...
/* Private Functions
 */
static BOOL WINAPI FLoadOffBkgBitmap(HWND hdlg, PLONG pdxOld, PLONG pdyOld, PLONG pdxNew, PLONG pdyNew);
static BOOL WINAPI FResizeDlgControls(HWND hdlg, LONG dxOld, LONG dyOld, LONG dxNew, LONG dyNew);
static VOID WINAPI ScaleDialogControl(HWND hdlg, HWND hwnd, LONG xNewSz, LONG xOldSz, LONG yNewSz, LONG yOldSz);
...
static BOOL WINAPI FDrawTransTextCtrl(HDC hdc, HWND hdlg, IDC idc, COLORREF crText);
static VOID WINAPI DrawTransparentBitmap(HDC hdc, HBITMAP hbmSrc, INT xDst, INT yDst, INT dx, INT dy, INT xSrc,
                                         INT ySrc, COLORREF crTrans);
...
**
****************************************************************************/
BOOL __export __loadds CALLBACK FOffBlbrdDlgProc(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        for (idc = IDC_OFFBBTXT1; (hwnd = GetDlgItem(hdlg, idc)) != hwndNull; ++idc)
        {
...
static BOOL WINAPI FDrawHeading(HDC hdc, HWND hdlg, IDC idc)
{
    ...
    DrawText(hdc, rgch, CbStrLen(rgch), &rc, DT_VCENTER | DT_SINGLELINE | DT_LEFT | DT_NOCLIP | DT_NOPREFIX);
...
static BOOL WINAPI FDrawTransTextCtrl(HDC hdc, HWND hdlg, IDC idc, COLORREF crText)
{
    ...
    iModSav = SetBkMode(hdc, TRANSPARENT);
    DrawText(hdc, rgch, CbStrLen(rgch), &rc, DT_LEFT | DT_NOCLIP | DT_NOPREFIX);
    SetTextColor(hdc, crSav);
    SetBkMode(hdc, iModSav);
    if (hfontSav != hfontNull)
        SelectObject(hdc, hfontSav);
...
static BOOL WINAPI FPaintBBmpOff(HWND hwnd, HBITMAP hbm)
{
    ...
    if ((hdc = BeginPaint(hwnd, &ps)) != NULL)
    {
        ...
        DrawTransparentBitmap(hdc, hbm, x, y, bm.bmWidth, bm.bmHeight, 0, 0, crTrans);
    }
 ...
 */
extern VOID WINAPI STF_CopyFile(SZ szFullPathSrc, SZ szFullPathDst, CMO cmo, BOOL fAppend, PSEC psecError);
...
static BOOL WINAPI FInitHeadFont(VOID)
{
    ...
    if (!fInitFont)
    {
            ...
            GetWindowsDirPath(vrgchOffTtfPath, sizeof vrgchOffTtfPath) != 0)
        {
             ...
             */
            for (sz = SzLastChar(rgchSrcTtf); *sz != '\\'; sz = SzPrevChar(rgchSrcTtf, sz))
                ;
            Assert(*sz == '\\');
            sz = SzNextChar(sz);
            ...
            STF_CopyFile(rgchSrcTtf, vrgchOffTtfPath, cmoOverwrite, 0, &secError);

            if (DoesFileExist(vrgchOffTtfPath, femExists))
            {
                RemoveFile(vrgchOffForPath, cmoForce);
                fFontReady = CreateScalableFontResource(0, vrgchOffForPath, vrgchOffTtfPath, NULL);
                if (!fFontReady)
    ...
    *lpch++ = '\0';
    ...
    /* block */
    {
        ...
        do
        {
            ...
            wPt -= 2;
        } while (hfont != NULL && wPt > 0 && ((int)sSize.cx > dxp || tm.tmHeight > dyp));
...
static HFONT WINAPI HfontSetTextFont(HWND hdlg, LONG dyOld, LONG dyNew)
{
    ...
    if ((hfontDlg = (HFONT)SendMessage(hdlg, WM_GETFONT, 0, 0L)) != hfontNull)
    {
        if (GetObject(hfontDlg, sizeof(LOGFONT), (LPSTR)&lFont))
        {
             ...
             *	mess us up.
             */
            lFont.lfHeight = (lFont.lfHeight * dyNew + (lFont.lfHeight < 0 ? 0 - dyOld + 1 : dyOld - 1)) / dyOld;

            lFont.lfWeight = FW_BOLD;
            if ((hfontDlg = CreateFontIndirect((LPLOGFONT)&lFont)) != hfontNull)
            {
                for (idc = IDC_OFFBBTXT1; GetDlgItem(hdlg, idc) != hwndNull; ++idc)
                {
                    Assert(idc < IDC_OFFBBTXTMAX);
                    SendDlgItemMessage(hdlg, idc, WM_SETFONT, (WPARAM)hfontDlg, 0L);
                }
            }
        }
    }
