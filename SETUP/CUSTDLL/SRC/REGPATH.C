...
CSZC cszcBadPath = "ObjID %u: GetPathFromReg: The following invalid destination path "
                   "was found in the registry:\n\n%s\n\nSetup will use the object's "
                   "default path instead.";
CSZC cszcBadPathUNC = "ObjID %u: GetPathFromReg: The following path was found in the "
...
#define SzDirFromPcamfd(pcamfd)                                                                                        \
    ((((PCAMFDDtor)pcamfd)->lpvPriv == lpvoidNull) ? szNull : ((PRPD)(((PCAMFDDtor)pcamfd)->lpvPriv))->szDir)
...
STATIC_FN RC PRIVATE RcParseGetRegPath(OR orCur, SZ szData, PHKEY phkeyRoot, PSZ pszKey, PSZ pszValueName);
...
STATIC_FN RC PRIVATE RcInitializeTrigger(PCD pcd, POD pod, SZ szData, PCAMFDInitializeTrigger pcamfd)
{
     ...
     */
    rc = RcParseGetRegPath(pod->or, szData, &hRootKey, &szKey, &szValueName);
    if (rc != rcOk)
        goto LExitInit;
    rc = rcDoDefault; /* Reset */

    if (GetRegKeyValue32(hRootKey, szKey, szValueName, &dwType, (PBYTE)rgchPath, sizeof rgchPath) <= 1)
    {
    ...
    if (FValidDstDir(rgchPath))
    {
        ...
        while ((prpd->szDir = (SZ)PbAlloc(CbStrLen(rgchPath) + 1)) == szNull)
        {
            if (!FHandleOOM())
            {
                rc = rcFail;
...
STATIC_FN HKEY PRIVATE HkeyFromSzRootKey(SZ szRootKey)
{
    ...
    else if (lstrcmpi(szRootKey, "HKEY_CLASSES_ROOT") == 0 || lstrcmpi(szRootKey, "CLASSES") == 0)
    {
        return (HKEY_CLASSES_ROOT);
    }
    else if (lstrcmpi(szRootKey, "HKEY_CURRENT_USER") == 0 || lstrcmpi(szRootKey, "CURRENT") == 0)
    {
        return (HKEY_CURRENT_USER);
    }
    else if (lstrcmpi(szRootKey, "HKEY_LOCAL_MACHINE") == 0 || lstrcmpi(szRootKey, "LOCAL") == 0)
    {
        return (HKEY_LOCAL_MACHINE);
    }
    else if (lstrcmpi(szRootKey, "HKEY_USERS") == 0 || lstrcmpi(szRootKey, "USERS") == 0)
    {
        return (HKEY_USERS);
    }
