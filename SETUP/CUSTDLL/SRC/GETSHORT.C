...
**
**	STF Syntax:
**		ObjID	Batch	Title	Descr	Type		Data						Bmp Id	Vital	Shared	DirChng	DestDir
*CheckDir	InstBy	InstData	InstDir *		ID#				CustomAction	"EXPSETUP.DLL","GetShortPath","ObjID#"
**
**	Author: ******
...
#ifdef _DEBUG
#define DEBUG_OUTPUT
const char camfText[][24] = {"camfInitializeObject",
            ...
            if (rcOk == rc)
            {
...
#ifdef DEBUG_OUTPUT
                {
                    ...
                    wsprintf(szBuf, "ObjID=%u\nShort='%s'\nLong='%s'\nTblDstDir='%s'\nDstDir='%s'\nInstallDir='%s'", or,
                             (LPSTR)szShortPathname, (LPSTR)camfdGetObjData.podRes->szDstDir, (LPSTR)pod->szTblDstDir,
                             (LPSTR)pod->szDstDir, (LPSTR)pod->szInstallDir);
