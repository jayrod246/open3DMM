...
RC FAR PASCAL DetectVideo(PCD pcd, POD pod, CAMF camf, PCAMFD pcamfd, SZ szData);
    ...
    case camfInitializeObject:
        if (!_tAnswer)
        {
            ...
            if (_tAnswer < 0)
            {
                ...
                {
                    tResult = DoMsgBox("Machine too slow.  Press OK to quit setup, or Cancel to continue setup.",
                                       rgchCaption, MB_OKCANCEL);
                }
...
long DibDraw(HDC hdc, int x, int y, PDIB pdib, int x0, int y0, LONG rop, UINT wUsage)
{
    ...
    f = StretchDIBits(hdc, x, y, dx0, dy0, x0, y0, dx0, dy0, lpBits, (LPBITMAPINFO)pdib, wUsage, rop);
...
#define RandPT(pt, rc)                                                                                                 \
    ((pt).x = rc.left + (rand() % (rc.right - rc.left))), ((pt).y = rc.top + (rand() % (rc.bottom - rc.top)))
