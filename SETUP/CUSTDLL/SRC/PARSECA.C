...
RC PUBLIC RcParseInfSectKey(OR orCur, SZ szData, PSZ pszInfSect, PSZ pszInfKey)
{
    ...
    if (!FValidInfKey(*pszInfKey) || !DoesInfSectionKeyExist(*pszInfSect, *pszInfKey))
    {
...
BOOL PUBLIC FValidGroupPlor(PCD pcd, PLOR plor, OR orGroup)
{
    ...
    for (ior = 0; ior < plor->cor; ior++)
    {
        ...
        if (or <= orGroup || or >= pcd->cObjectsMax || PodGetObjData(pcd, or) == podNull)
        {
            return (fFalse);
        }
...
LWalkStringReadingLOR:
    ...
    while (*szCur != chEos)
    {
        ...
        if (*szCur == chEos)
            break;
        szSep = szCur;
        while (*szSep != chEos && !FWhiteSpaceCh(*szSep) && *szSep != chLorSep)
        {
            if (!isdigit(*szSep))
            {
    ...
    if (plor == plorNull && cor > 0 && (plor = PlorAlloc(cor)) != plorNull)
    {
        Assert(plor->rgor != rgorNull);
        Assert(plor->cor == cor);
