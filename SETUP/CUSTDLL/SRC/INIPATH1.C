...
RC FAR PASCAL GetPathFromIni(PCD pcd, POD pod, CAMF camf, PCAMFD pcamfd, SZ szData);
...
STATIC_FN RC PRIVATE RcInitializeObject(PCD pcd, PCAMFDInitializeObject pcamfd, SZ szData);
...
RC FAR PASCAL GetPathFromIni(PCD pcd, POD pod, CAMF camf, PCAMFD pcamfd, SZ szData)
{
    ...
    case camfCalcDstDir:
        if (pcd->simSetupMode != simMaintM && pcd->simSetupMode != simAdminM)
        {
...
201		"Setup Error"	"Setup couldn't located a necessary Explora.ini entry"	CustomAction
"EXPSETUP.DLL","GetPathFromIni","Explora.ini,Paths,InstPath"
...
STATIC_FN RC PRIVATE RcInitializeObject(PCD pcd, PCAMFDInitializeObject pcamfd, SZ szData)
{
    ...
    if (pcamfd->lpvPriv == lpvoidNull && pcd->simSetupMode != simMaintM && pcd->simSetupMode != simAdminM)
    {
